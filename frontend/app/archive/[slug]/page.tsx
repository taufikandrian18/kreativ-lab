    ...
    <main>
      <section className="bg-k-paper text-k-black">
        <div className="section-shell">
          <p className="font-display text-k-red text-3xl tracking-tight">
            {project.archive_no}
          </p>
          ...
          {/* The opener card and the reel share the top row. The opener carries the
              ...
              than above it with the film buried further down the page. The row is its own
              grid: section-shell is not one, so these spans did nothing until it was, and
              the reel rendered full-width beneath the opener instead of beside it. */}
          <div className="mt-8 grid grid-cols-12 items-start gap-x-4 gap-y-8 sm:gap-x-8 lg:gap-x-12">
            {opener ? (
              <div
                className={reel ? "col-span-12 lg:col-span-7" : "col-span-12"}
              >
                <ProjectOpenerFigure
                  opener={opener}
                  alt={
                    opener.kind === "cms"
                      ? opener.image.alt ||
                        `${project.title} campaign photography`
          ...
          {project.scope.length > 0 ? (
            <ul data-scope-list className="mt-12">
              {project.scope.map((item) => (
                <li
                  key={item}
                  className="font-body border-k-black border-t-2 py-3 text-lg"
                >
                  {item}
                </li>
              ))}
